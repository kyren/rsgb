...
fn add_ca<C: Cpu>(cpu: &mut C, n: u8) {
    let v = cpu.get_register(ARegister);
    let mut flags = cpu.get_flags();

    let (v, h1, c1) = add8(v, n);
    let (v, h2, c2) = if flags.carry {
        add8(v, 1)
    } else {
        (v, false, false)
    };
...
fn sub_ca<C: Cpu>(cpu: &mut C, n: u8) {
    let v = cpu.get_register(ARegister);
    let mut flags = cpu.get_flags();

    let (v, h1, c1) = sub8(v, n);
    let (v, h2, c2) = if flags.carry {
        sub8(v, 1)
    } else {
        (v, false, false)
